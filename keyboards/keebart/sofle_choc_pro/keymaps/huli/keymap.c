...
[GAME] = LAYOUT_split_4x6_5(
  ...
  KC_B, KC_LSFT,  KC_Z,   KC_X,    KC_C,    KC_V,     _______,        _______, KC_N,    KC_M, KC_COMM,  KC_DOT, KC_SLSH,  KC_NUBS,
                    KC_LGUI, KC_GRV, KC_LALT, MO(FUNC), KC_SPC,      KC_ENT,  MO(EXTRA), KC_RCTL, KC_RALT, KC_RGUI
),
[FUNC] = LAYOUT_split_4x6_5(
  ...
  _______, KC_EXLM,  KC_QUESTION, KC_TILDE,  KC_AT, KC_PERC,                 RM_TOGG, RM_PREV, RM_NEXT, KC_LPRN, KC_RPRN, KC_PIPE,
...
void set_num_key_color(void) {
    ...
    set_key_color(false, (uint8_t[]) { 23 }, 1, 0, 100, 50);
    set_key_color(false, (uint8_t[]) { 27,28 }, 2, 100, 0, 0);
    set_key_color(false, (uint8_t[]) { 29 }, 1, 0, 100, 0);
};
...
void set_extra_key_color(void) {
    rgb_matrix_set_color_all(DEFAULT_RGB);
    set_key_color(false, centeredArrow, 4, 0, 100, 0);
};
